...
bool VideoReader::open(const char *filename) {
  ...
  int ret;
  if (ret = avformat_open_input(&formatContext_, filename, nullptr, nullptr), ret) {
    spdlog::error("Unable to open video file: {}", av_err2str(ret));
    return false;
  }
  avformat_find_stream_info(formatContext_, nullptr);
  spdlog::info("Opening file: {} with {} streams", filename, formatContext_->nb_streams);
...
bool VideoReader::setupVideoCodecContext() {
  ...
  if (videoStreamIndex_ == -1) {
    ...
    if (ret = avcodec_parameters_to_context(videoCodecContext_, videoCodecParams_), ret < 0) {
      spdlog::error("Unable to initialize video codec context: {}", av_err2str(ret));
      return false;
    }
    if (ret = avcodec_open2(videoCodecContext_, videoCodec_, nullptr), ret < 0) {
...
bool VideoReader::setupAudioCodecContext() {
  ...
  if (audioStreamIndex_ == -1) {
    ...
    if (ret = avcodec_parameters_to_context(audioCodecContext_, audioCodecParams_), ret < 0) {
...
bool VideoReader::findAVStreams() {
  ...
  videoStreamIndex_ = av_find_best_stream(formatContext_, AVMEDIA_TYPE_VIDEO, -1, -1, &codec, 0);
  if (videoStreamIndex_ <= -1) {
    spdlog::error("Video stream not found: {}", av_err2str(videoStreamIndex_));
    std::exit(1);
  } else {
    auto stream = formatContext_->streams[videoStreamIndex_];
    ...
    durationMilli = stream->duration * stream->time_base.num * 1000. / stream->time_base.den;
  ...
  audioStreamIndex_ = av_find_best_stream(formatContext_, AVMEDIA_TYPE_AUDIO, -1, -1, &codec, 0);
  if (audioStreamIndex_ <= -1) {
    spdlog::info("Audio stream not found: {}", av_err2str(audioStreamIndex_));
  } else {
    auto stream = formatContext_->streams[audioStreamIndex_];
    params = stream->codecpar;
    audioCodec_ = codec;
    audioCodecParams_ = params;
    audioStreamTimebase_ = stream->time_base;
...
void VideoReader::setWindow(SDL_Window *window) {
  curWindow = window;
}
